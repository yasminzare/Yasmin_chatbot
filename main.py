...
def tavilliy_search(subject: str):
    ...
    try:
        ...
        return f"No sources found, error: {e}"
...
def run_pipeline():
    while True:
        try:
            ...
            prompt_ = "Do you have any other questions? (y/n): "
            continue_prompt = input(prompt_).strip().lower()
...
def parse_output(response):
    try:
        tavily_response = tavilliy_search(
            response.content
        )  # Assuming response has a 'content' attribute
